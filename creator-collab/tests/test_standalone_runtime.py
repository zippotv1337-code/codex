...
class StandaloneRuntimeTests(unittest.TestCase):
    ...
    def test_runtime_task_scripts_are_bounded_and_owner_gated(self) -> None:
        ...
        self.assertIn("dispatch_live = true", config)
        self.assertIn("live_enabled = true", config)
        ...
        self.assertIn("live dispatch skipped by complete owner gate", scheduler)
        ...
        self.assertTrue(parsed["capabilities"]["official_instagram_publish"])
        self.assertTrue(parsed["capabilities"]["live_external_actions"])
