...
class StandaloneRuntimeTests(unittest.TestCase):
    ...
    def test_runtime_task_scripts_are_bounded_and_owner_gated(self) -> None:
        ...
        self.assertNotIn("publish-dispatch-due", scheduler)
        self.assertIn("creator_ops.local_ops idle", scheduler)
        ...
        self.assertNotIn("morning-run", scheduler)
        self.assertNotIn("creator_ops.cli", scheduler)
        self.assertIn("Script=(Join-Path $PSScriptRoot 'run_healthcheck.ps1')", installer)
        self.assertIn("OfficialInstagramPublish", helper)
        self.assertIn("LiveExternalActions", helper)
        ...
        self.assertEqual(parsed["publishing"]["adapter"], "unconfigured")
        self.assertFalse(parsed["capabilities"]["official_instagram_publish"])
        self.assertFalse(parsed["capabilities"]["live_external_actions"])
