...
function render(data) {
  ...
  const local=data.agents.find(a=>a.id==='local_ai')||{};const done=data.tasks.filter(t=>t.status==='DONE').length;const current=local.task||data.tasks.find(t=>t.status==='WORKING')?.title||'Kein aktiver Task';
  document.querySelector('#ops-proof').innerHTML=`<div><p class="eyebrow">Nachvollziehbarer Worker-Lauf</p><h2>Local AI / Qwen</h2><p><strong>${esc(done)} von ${esc(data.tasks.length)}</strong> Aufgaben abgeschlossen. Letzter Erfolg: <strong>${esc(when(local.last_success))}</strong>.</p><p>Aktueller/letzter Task: <strong>${esc(current)}</strong><br>Ergebnisse: <code>C:\\Zippoworkz\\Handoff</code></p></div><span class="ops-proof-state">${badge(local.status||'OFFLINE')}<small>läuft bewusst ohne sichtbares PowerShell-Fenster</small></span>`;
